...
function signOut() {
  ...
  console.log(`${firebase.auth().currentUser.displayName} signed out on ${ getLocalDateAndTime()}`)
  firebase.auth().signOut();
}

// Initiate firebase auth and pass in state observer
...
function saveMessage(messageText) {
  ...
  // Add a new message entry to the Firebase Database.
    var key = firebase.database().ref('/chatapp').child('messages').push().key
    return firebase.database().ref('/chatapp').child('messages/'+key).set({  //'/chatapp/messages/'
    name: getUserName(),
    message: messageText,
...
function saveImageMessage(file) {
  ...
  // 1 - We add a message with a loading icon that will get updated with the shared image.
    firebase.database().ref('/chatapp/messages/').push({
      name: getUserName(),
      imageUrl: LOADING_IMAGE_URL,
      profilePicUrl: getProfilePicUrl(),
      timestamp: getLocalDateAndTime()
    }).then(function(messageRef) {
      // console.log('this is the messageRef object returned:   ', messageRef)
      // 2 - Upload the image to Cloud Storage.
      var filePath = firebase.auth().currentUser.uid + '/' + messageRef.key + '/' + file.name;
      return firebase.storage().ref('chatapp-uploaded-images/' + filePath).put(file).then(function(fileSnapshot) {
  ...
  firebase.messaging().getToken()
    .then(function(currentToken) {
      if (currentToken) {
        ...
        firebase.database().ref('chatapp/fcmTokens').child(currentToken)
...
// Triggers auth state change for instance when the user signs-in or signs-out.
